...
# FUNÇÕES DE LEITURA E ESCRITA DO HIGH SCORE ********************************

def load_high_score():
    try:
        with open("high_score.json", "r") as file:
            data = json.load(file)
            return data.get("high_score", 0)  # Retorna o high_score do arquivo, ou 0 se não existir
    except FileNotFoundError:
        return 0  # Caso o arquivo não exista, retorna 0

def save_high_score(high_score):
    with open("high_score.json", "w") as file:
        json.dump({"high_score": high_score}, file)  # Salva o high_score no arquivo
...
def reset():
    ...
    if score >= high_score:
        high_score = score
        save_high_score(high_score)  # Salva o novo high_score
...
score = 0
high_score = load_high_score()  # Carrega o high_score ao iniciar o jogo
